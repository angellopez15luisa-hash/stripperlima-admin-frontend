...
import FormCatalogGalleryVideos from '@/components/ui/gallery-videos/FormCatalogGalleryVideos.vue'
...
// Estados locales para la gestión de la galería de videos y filtros
const catalogGalleryList = ref<CatalogGalleryVideo[]>([])
const originalCatalogBackup = ref<CatalogGalleryVideo[]>([])
...
const [titleHeader] = defineField('titleHeaderGalleryVideos')
const [descriptionHeader] = defineField('descriptionHeaderGalleryVideos')
...
const { mutate, isPending } = useMutation({
  ...
  onSuccess: (data, variables) => {
    ...
    queryClient.setQueryData(['general-settings'], (oldData: any) => {
      return {
        ...oldData,
        titleHeaderGalleryVideos: newValues.titleHeaderGalleryVideos,
        descriptionHeaderGalleryVideos: newValues.descriptionHeaderGalleryVideos,
        catalogGalleryVideos: newValues.catalogGalleryVideos,
      }
    })
  ...
  if (isEditing.value) {
    ...
    if (generalSetting.value) {
      resetForm(
        {
          values: {
            titleHeaderGalleryVideos: generalSetting.value.titleHeaderGalleryVideos || '',
            descriptionHeaderGalleryVideos:
              generalSetting.value.descriptionHeaderGalleryVideos || '',
            catalogGalleryVideos: generalSetting.value.catalogGalleryVideos || [],
          },
        },
        { force: true },
      )
...
const handleGlobalCreate = (newVideo: CatalogGalleryVideo) => {
  queryClient.setQueryData(['general-settings'], (oldData: any) => {
    if (!oldData) return oldData
    return {
      ...oldData,
      catalogGalleryVideos: [newVideo, ...(oldData.catalogGalleryVideos || [])],
    }
  })
}

function handleGlobalUpdate(id: CatalogGalleryVideo['id'], updatedVideo: CatalogGalleryVideo) {
  queryClient.setQueryData(['general-settings'], (oldData: any) => {
    if (!oldData) return oldData
    return {
      ...oldData,
      catalogGalleryVideos: oldData.catalogGalleryVideos.map((video: CatalogGalleryVideo) =>
...
function handleGlobalDelete(id: CatalogGalleryVideo['id']) {
  queryClient.setQueryData(['general-settings'], (oldData: any) => {
    if (!oldData) return oldData
    return {
      ...oldData,
      catalogGalleryVideos: oldData.catalogGalleryVideos.filter(
        (video: CatalogGalleryVideo) => video.id !== id,
      ),
    }
  })
}
...
const onSubmit = () => {
  ...
  mutate({
    ...
    data: {
      titleHeaderGalleryVideos: titleHeader.value,
      descriptionHeaderGalleryVideos: descriptionHeader.value,
      catalogGalleryVideos: catalogGalleryList.value,
    },
  })
}
...
watch(
  ...
  (newData) => {
    if (newData) {
      setValues({
        titleHeaderGalleryVideos: newData.titleHeaderGalleryVideos,
        descriptionHeaderGalleryVideos: newData.descriptionHeaderGalleryVideos,
      })

      catalogGalleryList.value =
        typeof newData.catalogGalleryVideos === 'string'
          ? JSON.parse(newData.catalogGalleryVideos)
          : newData.catalogGalleryVideos || []
    }
  },
  {
    deep: true,
...
<template>
  <div class="w-full p-6 space-y-6">
    <!-- CABECERA Y SECCIÓN: Mantenimiento de Textos de Videos -->
    <HeaderTitlesSlot>
      <template #title>Mantenimiento de Sección Videos</template>
      <template #description>
        Personaliza el título principal, descripción y gestiona el catálogo de videos del sitio web.
      </template>
      <template #button>
        <button
          @click="toggleEdit"
    ...
    <form class="space-y-6" @submit.prevent="onSubmit">
      ...
      <FormTextHeader
        ...
        title-field-name="titleHeaderGalleryVideos"
        desc-field-name="descriptionHeaderGalleryVideos"
        :errors="errors"
        :disabled="!isEditing"
      />

      <!-- Componente del catálogo de la galería de videos -->
      <FormCatalogGalleryVideos
        :videos="catalogGalleryList"
        :isEditing="isEditing"
      ...
      <div class="flex justify-end w-full pt-0">
        ...
        >
          ...
          <span>{{
            isPending ? 'Guardando...' : isEditing ? 'Guardar Cambios de Videos' : 'Guardar Cambios'
          }}</span>
        </button>
      </div>
    </form>
