...
import type { CatalogGalleryEvent } from '@/types/general-setting.ts'
import { EVENT_CATEGORY_DICTIONARY, getCategoryLabel } from '@/constants/categories.constant'
import ModalCatalogGalleryEvents from './ModalCatalogGalleryEvents.vue'

// Define las propiedades (props) que recibe este componente principal
const props = defineProps<{
  events: CatalogGalleryEvent[] // Arreglo con la lista de eventos del catálogo
...
const emit = defineEmits<{
  ...
  delete: [id: CatalogGalleryEvent['id']]
...
// Variable reactiva para almacenar la categoría seleccionada en el filtro desplegable
...
// Variable reactiva para almacenar temporalmente los datos del evento seleccionado al momento de editar
const selectedEventForEdit = ref<any>(null)

// Propiedad computada para obtener las categorías únicas presentes en los eventos actuales
// const availableCategories = computed(() => {
//   const categories = props.events.map((event) => event.category).filter((cat) => Boolean(cat))
...
const openCreateModal = () => {
  ...
  selectedEventForEdit.value = null // Limpia cualquier dato de edición anterior
}

// Función para abrir el modal en modo edición cargando los datos del evento seleccionado
const openEditModal = (event: CatalogGalleryEvent) => {
  modalMode.value = 'edit' // Cambia el modo del modal a edición
  isModalOpen.value = true // Abre el modal
  selectedEventForEdit.value = event // Asigna los datos del evento a editar
}
...
watch(
  ...
  (newDisabled) => {
    if (!newDisabled) {
      ...
      selectedCategory.value = 'Todas' // Restablece el filtro de categorías
    }
  },
)

const handleCreateEvent = (newEvent: CatalogGalleryEvent) => {
  emit('create', newEvent)
}

const handleUpdateEvent = (id: CatalogGalleryEvent['id'], updatedEvent: CatalogGalleryEvent) => {
  emit('update', id, updatedEvent)
}

const disabled = computed(() => !props.isEditing)

// Función para mover un evento una posición hacia atrás (arriba/izquierda) en el orden general
const moveUp = (eventId: CatalogGalleryEvent['id']) => {
  const globalIndex = props.events.findIndex((s) => s.id === eventId) // Busca el índice global del evento
  if (globalIndex > 0) {
    emit('move', globalIndex, globalIndex - 1) // Emite el evento de movimiento si no está al inicio
  }
}

// Función para mover un evento una posición hacia adelante (abajo/derecha) en el orden general
const moveDown = (eventId: CatalogGalleryEvent['id']) => {
  const globalIndex = props.events.findIndex((s) => s.id === eventId) // Busca el índice global del evento
  if (globalIndex !== -1 && globalIndex < props.events.length - 1) {
    emit('move', globalIndex, globalIndex + 1) // Emite el evento de movimiento si no está al final
  }
}

// Función asíncrona para manejar la eliminación de un evento mediante una alerta de confirmación (SweetAlert2)
const deleteEvent = async (id: CatalogGalleryEvent['id']) => {
  const dark = isCurrentDarkMode()
  const result = await Swal.fire({
    title: '¿Estás seguro?',
    text: 'Esta acción no se puede deshacer y el evento será eliminado.',
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    emit('delete', id) // Emite el evento 'delete' con el ID del evento
    Swal.fire({
      title: '¡Eliminado!',
      text: 'El evento ha sido borrado exitosamente.',
      icon: 'success',
      timer: 1500,
...
// Propiedad computada que filtra la lista de eventos según el texto de búsqueda, estado y categoría seleccionada
const filteredEvents = computed(() => {
  ...
  const categoryFilter = selectedCategory.value // Almacena el valor actual del filtro de categoría

  return props.events.filter((event) => {
    // Comprueba si el nombre o la descripción coinciden con el texto buscado
    ...
    if (statusFilter !== 'Todos') {
      ...
      matchesStatus = event.active === isTargetActive
    }

    let matchesCategory = true
    // Evalúa el filtro por categoría si no está en 'Todas'
    if (categoryFilter !== 'Todas') {
      matchesCategory = event.category === categoryFilter
    }

    return matchesSearch && matchesStatus && matchesCategory // Retorna solo los elementos que cumplen todas las condiciones
  })
})
...
// Propiedad computada que corta el arreglo de eventos filtrados para mostrar únicamente los elementos de la página actual
...
// Observador múltiple que reinicia la página actual a 1 cada vez que cambia el texto de búsqueda, estado o categoría
...
<template>
  ...
  >
    ...
    >
      ...
      <div>
        <h2 class="text-sm font-bold text-slate-800 dark:text-white">Catálogo de Eventos</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">
          Filtra, agrega o administra las tarjetas de eventos de la landing.
        </p>
      </div>
      ...
      <div class="flex items-center gap-3 w-full md:w-auto justify-end flex-wrap">
        ...
        <!-- Contenedor del menú desplegable para filtrar por categoría -->
        <div class="relative w-full sm:w-44">
          ...
          >
            ...
            <option
              v-for="(label, key) in EVENT_CATEGORY_DICTIONARY as Record<string, string>"
              :key="key"
              :value="key"
    ...
    <!-- Cuadrícula (Grid) que renderiza de forma iterativa cada tarjeta de evento paginado -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      >
        <!-- Sección superior de la tarjeta: Imagen del evento y etiqueta (badge) de estado -->
            ...
            >
              <span
                :class="[
                  ...
                  event.active ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400',
                ]"
              ></span>
        ...
        <!-- Sección de contenido textual de la tarjeta (Título y categoría) -->
        <div class="p-4 space-y-3 flex-1 flex flex-col justify-between">
          ...
          >
            <!-- Grupo de botones para cambiar el orden horizontal de los eventos -->
            <div class="flex items-center gap-1">
              <button
                @click="moveUp(event.id!)"
                :disabled="disabled || props.events.findIndex((m) => m.id === event.id) === 0"
                type="button"
              ...
              <button
                ...
                :disabled="
                  disabled ||
                  props.events.findIndex((s) => s.id === event.id) === props.events.length - 1
                "
                type="button"
                class="p-1.5 px-3 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition"
            ...
            <!-- Grupo de botones para editar o eliminar el evento específico -->
            <div class="flex items-center gap-1">
              <button
    ...
    <!-- Componente Modal utilizado para la creación y edición de elementos del catálogo de eventos -->
    <ModalCatalogGalleryEvents
      v-model:isOpen="isModalOpen"
      v-model:model="selectedEventForEdit"
      :mode="modalMode"
