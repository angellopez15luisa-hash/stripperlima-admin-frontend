...
import type { CatalogGalleryModel } from '@/types/general-setting.ts'
import { CATEGORY_DICTIONARY } from '@/constants/categories.constant'
import ModalCatalogGalleryModel from './ModalCatalogGalleryModel.vue'

// Define las propiedades (props) que recibe este componente principal
const props = defineProps<{
  models: CatalogGalleryModel[] // Arreglo con la lista de servicios del catálogo
...
const emit = defineEmits<{
  ...
  delete: [id: CatalogGalleryModel['id']]
...
const selectedModelForEdit = ref<any>(null)

// Propiedad computada para obtener las categorías únicas presentes en los modelos actuales
const availableCategories = computed(() => {
  const categories = props.models.map((model) => model.category).filter((cat) => Boolean(cat))
...
const openCreateModal = () => {
  ...
  selectedModelForEdit.value = null // Limpia cualquier dato de edición anterior
}

// Función para abrir el modal en modo edición cargando los datos del servicio seleccionado
const openEditModal = (model: CatalogGalleryModel) => {
  modalMode.value = 'edit' // Cambia el modo del modal a edición
  isModalOpen.value = true // Abre el modal
  selectedModelForEdit.value = model // Asigna los datos del servicio a editar
}
...
const handleCreateModel = (newModel: CatalogGalleryModel) => {
  emit('create', newModel)
}

const handleUpdateModel = (id: CatalogGalleryModel['id'], updatedModel: CatalogGalleryModel) => {
  emit('update', id, updatedModel)
}
...
const moveUp = (modelId: CatalogGalleryModel['id']) => {
  const globalIndex = props.models.findIndex((s) => s.id === modelId) // Busca el índice global del servicio
  if (globalIndex > 0) {
...
const moveDown = (modelId: CatalogGalleryModel['id']) => {
  const globalIndex = props.models.findIndex((s) => s.id === modelId) // Busca el índice global del servicio
  if (globalIndex !== -1 && globalIndex < props.models.length - 1) {
...
const deleteModel = async (id: CatalogGalleryModel['id']) => {
  const dark = isCurrentDarkMode()
  const result = await Swal.fire({
    title: '¿Estás seguro?',
    text: 'Esta acción no se puede deshacer y el modelo será eliminado.',
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    ...
    Swal.fire({
      title: '¡Eliminado!',
      text: 'El modelo ha sido borrado exitosamente.',
      icon: 'success',
      timer: 1500,
...
// Propiedad computada que filtra la lista de servicios según el texto de búsqueda, estado y categoría seleccionada
const filteredModels = computed(() => {
  ...
  return props.models.filter((model) => {
    ...
    if (statusFilter !== 'Todos') {
      ...
      matchesStatus = model.active === isTargetActive
    }

    let matchesCategory = true
    // Evalúa el filtro por categoría si no está en 'Todas'
    if (categoryFilter !== 'Todas') {
      matchesCategory = model.category === categoryFilter
    }
...
<template>
  ...
  >
    ...
    >
      ...
      <div>
        <h2 class="text-sm font-bold text-slate-800 dark:text-white">Catálogo de Modelos</h2>
      ...
      >
            ...
            >
              ...
              <span
                :class="[
                  ...
                  model.active ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400',
                ]"
              ></span>
        ...
        <div class="p-4 space-y-3 flex-1 flex flex-col justify-between">
          <div class="space-y-1">
            ...
            >
              {{ CATEGORY_DICTIONARY[model.category!] || model.category }}
            </span>
          ...
          >
            ...
            <div class="flex items-center gap-1">
              <button
                @click="moveUp(model.id!)"
                :disabled="disabled || props.models.findIndex((m) => m.id === model.id) === 0"
                type="button"
              ...
              <button
                @click="moveDown(model.id!)"
                :disabled="
                  disabled ||
                  props.models.findIndex((s) => s.id === model.id) === props.models.length - 1
                "
                type="button"
                class="p-1.5 px-3 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition"
    ...
    <ModalCatalogGalleryModel
      v-model:isOpen="isModalOpen"
      v-model:model="selectedModelForEdit"
      :mode="modalMode"
